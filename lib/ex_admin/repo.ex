defmodule ExAdmin.Repo do
  ...
  def changeset_attributes_for(%Changeset{} = changeset, resource, params) do
    {new_changeset, fields} =
      Enum.reduce(insert_or_update_attributes_for(resource, params), {changeset, []}, fn {cs, fun,
                                                                                          field},
                                                                                         {acc,
                                                                                          fields} ->
        cs = ExAdmin.Changeset.set_data(cs, params)

        {Changeset.update(acc, valid?: cs.valid?, dependents: {cs, fun}, errors: cs.errors),
         fields ++ [{field, cs}]}
      end)
  ...
  def changeset_collection(%Changeset{} = changeset, resource, params) do
    {new_changeset, fields} =
      Enum.reduce(insert_or_update_collection(resource, params), {changeset, []}, fn {coll, fun,
                                                                                      field},
                                                                                     {acc, fields} ->
  ...
  def set_changeset_collection(fields, %{changeset: %{data: data}} = changeset) do
    data =
      Enum.reduce(fields, data, fn {k, v}, acc ->
  ...
  def set_dependents(changeset, list) do
    fields =
      Helpers.group_by(list, fn {key, _val} -> key end)
      |> Enum.map(fn {k, v} ->
  ...
  def do_attributes_for(resource, model, id, params, inx)
      when id > 100_000_000_000 or id == nil do
    {assoc_model, join_model} = get_assoc_model(resource, model)

    cs =
      assoc_model.changeset(assoc_model.__struct__, params)
  ...
  def do_attributes_for(resource, model, _id, %{_destroy: "1"} = params, inx) do
    ...
    cs =
      assoc_model.changeset(assoc_resource, params)
      |> attributes_for_translate_errors(resource, model, inx)
